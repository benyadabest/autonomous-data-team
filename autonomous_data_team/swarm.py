...
def run_attachment_swarm(
    ...
    dataset_reports = [
        _run_dataset_workers(run.run_id, run_dir, attachment, message, settings, store)
...
def run_local_dataset_swarm(
    ...
    run = store.create_run(
        mode="local_dataset",
        metadata={"path": path, "subject": pseudo_message.subject},
    )
    run_dir = settings.runs_dir / run.run_id
    attachments_dir = run_dir / "attachments"
    ...
    report = _run_dataset_workers(run.run_id, run_dir, attachment, pseudo_message, settings, store)
...
def _email_body(dataset_reports: list[dict[str, str]]) -> str:
    ...
    for report in dataset_reports:
        lines.extend(
            [
                f"- {report['dataset_name']}: {report['summary']}",
